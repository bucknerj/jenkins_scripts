job('checkout-ber') {
  displayName('checkout ber')
  description('use git to checkout Bernies branch from our gitlab server')
  multiscm {
    git {
      branch('bernie-changes-2022-01-14.update.withmaster')
      remote {
        name('origin')
...
cmakeBuilds.each {
  def current = it
// begin build
job("build-ber-${current.name}") {
  displayName("build ber ${current.name}")
...
// begin test
job("test-ber-${current.name}") {
  displayName("test ber ${current.name}")
  description("${current.name}\n${current.build}\n${current.test}")
  multiscm {
    git {
      branch('master')
  ...
  triggers {
    upstream("build-ber-${current.name}")
  }
  steps {
